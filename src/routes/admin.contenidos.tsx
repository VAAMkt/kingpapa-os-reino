...
import { deletePost, listAllPosts, updatePost } from "@/lib/posts";
...
function ContenidosList() {
  ...
  useEffect(() => {
    if (!loading && !hasAnyRole(["super_admin", "editor"])) {
                ...
                {posts.map((p) => (
                  <tr key={p.id} className="border-t-2 border-kp-ink/10">
                    ...
                    <td className="p-3 text-right space-x-2 whitespace-nowrap">
                      ...
                      <Link to="/admin/contenidos/$id" params={{ id: p.id }} className="underline text-xs">Editar</Link>
                      <button
